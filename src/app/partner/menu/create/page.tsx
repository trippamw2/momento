...
const EXPERIO_CATEGORIES = ["Date", "Chill", "Celebrate", "Escape"] as const;
...
export default function CreateListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/partner/listings/${data.id}`);
      } else {
        const data = await res.json();
      ...
      <div className="pt-24 pb-20">
        <div className="max-w-lg mx-auto px-4 sm:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="/" className="px-6 py-3 rounded-xl bg-[#FF0F73] text-white font-semibold text-body-sm hover:shadow-[0_4px_16px_rgba(255,15,115,0.3)] transition-all">
              Back to Home
            </a>
          </div>
        </div>
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto px-4 sm:px-8 pt-24 pb-20 text-center">
        <div className="w-16 h-16 rounded-full bg-[#FF0F73]/10 flex items-center justify-center mx-auto mb-5">
          <svg className="w-8 h-8 text-[#FF0F73]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h1 className="text-heading-xl font-bold text-[#F1F5F9] mb-3">Draft Created Successfully</h1>
        <p className="text-[#64748B] text-body mb-6">
          Your experience draft has been created. Continue editing to add photos, itinerary, pricing, and more.
        </p>
        <a
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-8 pt-24 pb-20">
      <div className="mb-8">
        <h1 className="text-display-sm font-bold text-[#F1F5F9] mb-2">Create New Listing</h1>
        <p className="text-[#64748B] text-body-lg">
          Quick draft creation — add full details (photos, itinerary, pricing, etc.) in the listing editor after creation.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {submitting ? (
            ...
            "Create Draft & Edit Details"
          )}
        </button>
      </form>

      <div className="mt-8 text-center">
        <p className="text-body-sm text-[#64748B] mb-3">Want to add all details now?</p>
        <a
          href="/partner/listings/new"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-white/[0.08] text-[#F1F5F9] font-semibold text-body-sm hover:bg-white/5 transition-colors"
        >
          Use Full Listing Editor →
        </a>
      </div>
